...
|---|---|---|
| `generate_aggregate_tie.py` | Generates `src/AggregateTie.generated.hpp`'s `TieMembers()` overloads (N = 0..`MAX_FIELDS`, 64 by default) — one per structured-binding arity, since arity can't be templated in standard C++20. A one-time/rare-regeneration generator, not run during a normal build; its output is checked in. See docs/adr/0001. | `python3 tools/generate_aggregate_tie.py > src/AggregateTie.generated.hpp` (only after changing `MAX_FIELDS`) |
