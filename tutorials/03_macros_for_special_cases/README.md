...
covers the cases where `REFLECT_CLASS_BEGIN` remains the deliberate,
hand-written choice instead, per
[docs/adr/0001-reflection-generation-strategy.md](../../docs/adr/0001-reflection-generation-strategy.md)'s
revision (enums are their own case — see
[`tutorials/05_enums`](../05_enums/README.md)):

1. **A direct fixed-size array member** — automatic reflection's
...
2. **Real, distinguishable names/identity** — automatically reflected
   ...
   layout).
3. **Non-aggregate types** (`BoundedCounter`: a constructor and a
